...
HRESULT ThirdHomework::Initialize()
{
	...
	for (int i = 0; i < MONSTERATONCE; i++)
	{
		DWORD enemyStateTransitions[][3] = {
			...
			{ Monster::STATE_STAND, Monster::EVENT_DUBIOUS, Monster::STATE_MOVE },
			{ Monster::STATE_MOVE, Monster::EVENT_FINDTARGET, Monster::STATE_FOLLOW },
			{ Monster::STATE_MOVE, Monster::EVENT_STOPWALK, Monster::STATE_STAND },
			{ Monster::STATE_ATTACK, Monster::EVENT_LOSTTARGET, Monster::STATE_STAND },
			...
			{ Monster::STATE_FOLLOW, Monster::EVENT_LOSTTARGET, Monster::STATE_STAND } };
		g_MonsterVector.push_back(new Monster(Monster::TYPE_AI, enemyStateTransitions, 11));
	}
	...
	if (!g_soundManager->add(name, &id)) //id=0부터 시작함.
		return FALSE;
...
void Move()
{
	if (g_MainCharacter->GetAlive())
	{
		if (g_MainCharacter->m_IsMoving == false && g_MainCharacter->m_AnimTime[g_MainCharacter->m_AnimPose] == 0.f)
		{
			...
			g_MainCharacter->m_AnimPose = (int)state::idle;
		}


		if ((GetAsyncKeyState(VK_LEFT) || GetAsyncKeyState(VK_RIGHT) || GetAsyncKeyState(VK_UP) || GetAsyncKeyState(VK_DOWN)) != 0)
		{
			if (g_MainCharacter->m_IsMoving == false)
				g_MainCharacter->m_AnimPose = (int)state::step;
...
void HitCheck(std::vector<Bullet*> bv, std::vector<Monster*> mv, ID2D1SolidColorBrush* bulletBrush)
{
	std::vector<Bullet*>::iterator iterB = bv.begin();
	std::vector<Monster*>::iterator iterM = mv.begin();

	for (; iterB != bv.end(); iterB++)
	{
		if ((*iterB)->m_IsFlying == true)
		{
			for (; iterM != mv.end(); iterM++)
			{
...
HRESULT ThirdHomework::OnRender()
{
	...
	if (SUCCEEDED(hr))
	{
		...
		if (g_MainCharacter->m_IsShooting == true && g_MainCharacter->m_IsMoving == false)
			g_MainCharacter->m_AnimPose = (int)state::shoot;
		else if (g_MainCharacter->m_IsShooting == true && g_MainCharacter->m_IsMoving == true)
			g_MainCharacter->m_AnimPose = (int)state::runningShoot;
		...
		if (g_MainCharacter->m_AnimTime[g_MainCharacter->m_AnimPose] > 2.f && g_MainCharacter->m_AnimPose != (int)state::idle)
		{
			g_MainCharacter->m_AnimTime[g_MainCharacter->m_AnimPose] = 0.f;
			if (g_MainCharacter->m_preAnimPose == (int)state::idle || g_MainCharacter->m_preAnimPose == (int)state::step || g_MainCharacter->m_preAnimPose == (int)state::run)
				g_MainCharacter->m_AnimPose = (g_MainCharacter->m_preAnimPose + 1) % 3;
			else if (g_MainCharacter->m_preAnimPose == (int)state::shoot)
				g_MainCharacter->m_AnimPose = (int)state::idle;
			else
				g_MainCharacter->m_AnimPose = (g_MainCharacter->m_preAnimPose + 1) % 7;

			if (g_MainCharacter->m_AnimPose == (int)state::idle)
				g_MainCharacter->m_AnimPose = (int)state::step;
		...
		wsprintf(EXP,L"HP:%d LV:%d EXP:%d",g_MainCharacter->m_HP,g_MainCharacter->GetLevel(),g_MainCharacter->GetEXP());
